Shader "GAPH Custom Shader/Dissolve Texture/Dissolve Texture(MaskAnimation)"{
	Properties{
		...
		SubShader{
				...
				{
					half2 distort = UnpackNormal(tex2D(_NormalTex, i.texcoord2 * 1.0f+(_Speed*_Time.x/20 * _DistortTimeFactor))).rg; // Get distort info from normal texture.
					distort += UnpackNormal(tex2D(_NormalTex, i.texcoord2 * 1.0f - (_Speed*_Time.x / 20 * _DistortTimeFactor) + float2(0.5f,0.15f))).rg;

                    half4 tex = tex2D(_MainTex, i.texcoord.xy + distort.x * _DistortFactor - (_Speed * _Time/20)); //Animate main texture using time and distort value.
                    tex *= tex2D(_MainTex, i.texcoord.xy + distort.y * _DistortFactor + (_Speed * _Time/20));
                    
					half4 tex2 = tex2D(_MainTex, i.texcoord.xy + distort.xy * _DistortFactor + (_Speed * _Time/10));
					tex2 *= tex2D(_MainTex, i.texcoord.xy + distort.xy * _DistortFactor - (_Speed * _Time / 10));
                    ...
                    half4 mask = saturate(tex2D(_Mask,i.uvmask)-_CutOut); // Get mask info with cutout value for cutout texture 
